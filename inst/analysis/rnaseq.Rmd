...
withr::with_dir(here::here(), {
  rnaseq_pca <- tar_read(rnaseq_pca)
  rnaseq_different_differences <- tar_read(rnaseq_different_differences)
  rnaseq_gsea_plot <- tar_read(rnaseq_gsea_plot)
})
```
...
```{r}
head(rnaseq_different_differences, 20) %>% 
  my_kable()
```
