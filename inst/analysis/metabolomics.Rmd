...
pcaMethods::pca(t(SummarizedExperiment::assay(raw)), scale = "uv", center = TRUE) %>% 
  ...
  aes(
    x = PC1, 
    y = PC2, 
    shape = interaction(type, treatment, oxygen, sep = "|"),
    color = replicate
  ) + 
  geom_point()
```
...
```{r}
pcaMethods::pca(t(SummarizedExperiment::assay(clean)), scale = "uv", center = TRUE) %>% 
  pcaMethods::scores() %>% 
  merge(SummarizedExperiment::colData(clean), by = 0) %>% 
  ggplot() +
  aes(
    x = PC1, 
    y = PC2, 
